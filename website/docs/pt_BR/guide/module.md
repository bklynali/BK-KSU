...
O BK-KSU fornece um mecanismo de módulo que consegue modificar o diretório do sistema enquanto mantém a integridade da partição do sistema. Esse mecanismo é conhecido como "sem sistema".

O mecanismo de módulos do BK-KSU é quase o mesmo do Magisk. Se você já está familiarizado com o desenvolvimento de módulos Magisk, o desenvolvimento de módulos BK-KSU é muito semelhante. Você pode pular a introdução dos módulos abaixo e só precisa ler [Diferenças com Magisk](difference-with-magisk.md).

::: warning METAMODULE NECESSÁRIO APENAS PARA MODIFICAÇÃO DE ARQUIVOS DO SISTEMA
BK-KSU usa uma arquitetura [metamodule](metamodule.md) para montar o diretório `system`. **Somente se seu módulo precisar modificar arquivos `/system`** (via diretório `system`), você precisa instalar um metamodule (como [meta-overlayfs](https://github.com/tiann/BK-KSU/releases)). Outros recursos de módulos como scripts, regras sepolicy e system.prop funcionam sem um metamodule.
:::

## WebUI

Os módulos do BK-KSU suportam a exibição de interfaces e a interação com os usuários. Para mais detalhes, consulte a [documentação do WebUI](module-webui.md).

## Configuração de Módulo

O BK-KSU fornece um sistema de configuração integrado que permite que os módulos armazenem configurações de chave-valor persistentes ou temporárias. Para mais detalhes, consulte a [documentação de Configuração de Módulo](module-config.md).

## BusyBox

O BK-KSU vem com um recurso binário BusyBox completo (incluindo suporte completo ao SELinux). O executável está localizado em `/data/adb/ksu/bin/busybox`. O BusyBox do BK-KSU suporta "ASH Standalone Shell Mode" alternável em tempo de execução. O que este Modo Autônomo significa é que ao executar no shell `ash` do BusyBox, cada comando usará diretamente o miniaplicativo dentro do BusyBox, independentemente do que estiver definido em `PATH`. Por exemplo, comandos como `ls`, `rm`, `chmod` **NÃO** usarão o que está em `PATH` (no caso do Android, por padrão será `/system/bin/ls`, `/system/bin/rm` e `/system/bin/chmod` respectivamente), mas em vez disso chamará diretamente os miniaplicativos internos do BusyBox. Isso garante que os scripts sempre sejam executados em um ambiente previsível e sempre tenham o conjunto completo de comandos, independentemente da versão do Android em que estão sendo executados. Para forçar um comando a **NÃO** usar o BusyBox, você deve chamar o executável com caminhos completos.

Cada script shell executado no contexto do BK-KSU será executado no shell `ash` do BusyBox com o Modo Autônomo ativado. Para o que é relevante para desenvolvedores terceirizados, isso inclui todos os scripts de inicialização e scripts de instalação de módulos.

Para aqueles que desejam usar o recurso Modo Autônomo fora do BK-KSU, existem 2 maneiras de ativá-los:
...
Para garantir que todos os shells `sh` subsequentes executados também sejam executados no Modo Autônomo, a opção 1 é o método preferido (e é isso que o BK-KSU e o gerenciador do BK-KSU usam internamente), pois as variáveis ​​de ambiente são herdadas para os subprocesso.

::: tip DIFERENÇAS COM MAGISK
O BusyBox do BK-KSU agora está usando o arquivo binário compilado diretamente do projeto Magisk. **Obrigado ao Magisk!** Portanto, você não precisa se preocupar com problemas de compatibilidade entre scripts BusyBox no Magisk e BK-KSU porque eles são exatamente iguais!
:::

## Módulos BK-KSU

Um módulo BK-KSU é uma pasta colocada em `/data/adb/modules` com a estrutura abaixo:
...
│   │
│   ├── skip_mount          <--- Se existir, o BK-KSU não montará sua pasta de sistema
...
|   ├── uninstall.sh        <--- Este script será executado quando o BK-KSU remover seu módulo
|   ├── action.sh           <--- Este script será executado quando o usuário clicar no botão Ação no BK-KSU
...
::: tip DIFERENÇAS COM MAGISK
O BK-KSU não possui suporte integrado para o Zygisk, portanto não há conteúdo relacionado ao Zygisk no módulo. No entanto, você pode usar [ZygiskNext](https://github.com/Dr-TSNG/ZygiskNext) para suportar módulos Zygisk. Neste caso, o conteúdo do módulo Zygisk é idêntico ao suportado pelo Magisk.
:::

### module.prop

`module.prop` é um arquivo de configuração para um módulo. No BK-KSU, se um módulo não contiver este arquivo, ele não será reconhecido como um módulo. O formato deste arquivo é o seguinte:
...
::: tip DIFERENÇAS COM MAGISK
Você pode usar a variável de ambiente `KSU` para determinar se um script está sendo executado no BK-KSU ou Magisk. Se estiver executando no BK-KSU, esse valor será definido como `true`.
:::
...
Você também pode declarar uma variável chamada `REMOVE` contendo uma lista de diretórios em `customize.sh` para executar operações de remoção, e o BK-KSU executará automaticamente `mknod <TARGET> c 0 0` nos diretórios correspondentes do módulo. Por exemplo:
...
Você pode declarar uma variável chamada `REPLACE` em seu arquivo `customize.sh`, que inclui uma lista de diretórios a serem substituídos, e o BK-KSU executará automaticamente as operações correspondentes em seu diretório de módulo. Por exemplo:
...
O BK-KSU fornece um mecanismo para injetar diretivas RC customizadas do Android Init no `init.rc` do sistema. Isso permite que os módulos registrem serviços Android customizados, definam gatilhos de propriedade ou executem outras ações de linguagem Init sem modificar a partição do sistema.

Durante a inicialização, o módulo do kernel BK-KSU intercepta as chamadas de sistema `read()` e `fstat()`. Quando o processo init do Android lê `/system/etc/init/hw/init.rc`, o BK-KSU adiciona de forma transparente o conteúdo RC customizado ao final do arquivo. O processo init analisa essas diretivas injetadas exatamente como o conteúdo original do init.rc.
...
Um instalador do módulo BK-KSU é um módulo BK-KSU empacotado em um arquivo ZIP que pode ser atualizado no gerenciador do BK-KSU. O instalador do módulo BK-KSU mais simples é apenas um módulo BK-KSU compactado como um arquivo ZIP.
...
::: warning AVISO
O módulo BK-KSU **NÃO** é compatível para instalação no Recovery personalizado!
:::
...
O script `customize.sh` é executado no shell BusyBox `ash` do BK-KSU com o Modo Autônomo ativado. As seguintes variáveis ​​e funções estão disponíveis:

#### Variáveis

- `KSU` (bool): uma variável para marcar que o script está sendo executado no ambiente BK-KSU, e o valor desta variável sempre será `true`. Você pode usá-lo para distinguir entre BK-KSU e Magisk.
- `KSU_VER` (string): a string da versão do BK-KSU atualmente instalado (ex.: `v0.4.0`).
- `KSU_VER_CODE` (int): o código da versão do BK-KSU atualmente instalado no espaço do usuário (ex.: `10672`).
- `KSU_KERNEL_VER_CODE` (int): o código da versão do BK-KSU atualmente instalado no espaço do kernel (ex.: `10672`).
- `BOOTMODE` (bool): sempre será `true` no BK-KSU.
...
- `KSU_UAPI_VER` (int): a versão UAPI do espaço do usuário do BK-KSU (ksud) (ex.: `2`). Esta versão é incrementada quando há alterações que quebram a compatibilidade no driver do kernel, e pode ser usada pelos módulos para verificar compatibilidade.
- `KSU_RUNTIME_MODE` (string): o modo de execução atual do BK-KSU. Os valores possíveis são `built-in` (modo GKI, compilado no kernel), `lkm` (carregado como módulo do kernel na inicialização) ou `late-load` (carregado como módulo do kernel após a inicialização).
- `KSU_LATE_LOAD` (int?): se o BK-KSU foi carregado tardiamente após a inicialização, esta variável é definida como `1`; caso contrário, não é definida.

::: warning AVISO
No BK-KSU, `MAGISK_VER_CODE` é sempre `25200` e `MAGISK_VER` é sempre `v25.2`. Por favor, não use essas duas variáveis ​​para determinar se ele está sendo executado no BK-KSU ou não.
:::
...
No BK-KSU, os scripts são divididos em dois tipos com base em seu modo de execução: modo post-fs-data e modo de serviço late_start.
...
No BK-KSU, os scripts de inicialização são divididos em dois tipos com base no local de armazenamento: scripts gerais e scripts de módulo.
...
Todos os scripts de inicialização serão executados no shell BusyBox `ash` do BK-KSU com o Modo Autônomo ativado.

### Explicação do processo de scripts de inicialização

A seguir está o processo de inicialização relevante para o Android (algumas partes foram omitidas), que inclui a operação do BK-KSU (com asteriscos iniciais) e pode ajudá-lo a entender melhor o propósito desses scripts de módulo:

```txt
0. Bootloader (nada nesta tela)
load patched boot.img
load kernel:
    - Modo GKI: kernel GKI com BK-KSU integrado
...
read init.rc
  *initrc injection: Kernel hook appends BK-KSU core RC and module modules.rc to init.rc
...
...
Além do fluxo de inicialização padrão descrito acima, o BK-KSU suporta um **modo late-load** para cenários de LKM (Loadable Kernel Module). Neste modo, o módulo do kernel BK-KSU é carregado **após o sistema ter sido totalmente iniciado**, em vez de durante o processo init.

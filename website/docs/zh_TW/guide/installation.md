...
1. `LKM`：使用**可載入核心模組**（LKM）的方式載入到裝置核心中，不會替換掉裝置原有的核心。
2. `GKI`：使用**通用核心鏡像**（GKI）取代掉裝置原有的核心。
...
取得官方韌體的方法有很多，如果你的裝置支援 `fastboot boot`，那麼我們最推薦以及最簡單的方法是使用 `fastboot boot` 臨時啟動 KernelSU 提供的 GKI 核心，然後安裝管理器，最後在管理器中直接安裝；這種方法不需要你手動下載官方韌體，也不需要你手動提取 boot。
...
:::tip 備份原廠 boot 映像
使用管理器的「直接安裝」可自動備份原廠 boot（或 init_boot）映像，以便在增量 OTA 時臨時還原。請注意，只有當前分區尚未被 KernelSU 修補過時，才會建立此備份。
備份映像的 SHA1 會寫入修補後的 boot 映像中，備份檔則儲存在 `/data/adb/ksu/ksu_backup_$SHA1`。
當你使用管理器的「解除安裝 → 恢復原廠映像檔」功能時，如果目前修補映像中記錄的 SHA1 有對應的備份檔案，便會直接進行還原。
自 3.2.6 起，如果是在沒有 root 的情況下首次安裝 KernelSU，在透過「選擇檔案」修補 boot 映像後，可選擇「備份為原廠映像」。此時備份映像會儲存在管理器的內部儲存空間中；刷入並啟動修補後的映像後，首次開啟管理器時會自動將該備份移動到 `/data/adb/ksu`。
:::

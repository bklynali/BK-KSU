...
从 [GitHub Releases](https://github.com/bklynali/BK-KSU/releases) 下载 KernelSU 管理器应用，然后将应用程序安装到设备并打开：
...
1. `LKM`：使用**可加载内核模块**（LKM）的方式加载到设备内核中，不会替换掉设备原有的内核。
2. `GKI`：使用**通用内核镜像**（GKI）替换掉设备原有的内核。
...
:::tip 备份原厂 boot 镜像
使用管理器的「直接安装」可自动备份原厂 boot （或 init_boot）镜像，以备增量 OTA 时临时还原。请注意，这种情况下只有当前分区没有被 KernelSU 修补过，才会产生备份。
备份镜像的 SHA1 值会保存到修补的 boot 镜像中，备份的文件存储在 `/data/adb/ksu/ksu_backup_$SHA1` 。
当使用管理器的「卸载 → 还原原厂镜像」功能时，如果当前修补的镜像记录的 SHA1 存在对应的备份文件，则会直接恢复。
自 3.2.6 起，如果是在没有 Root 的情况下首次安装 KernelSU ，使用「选择并修补一个文件」方式修补 boot 镜像后可选择「备份为原厂镜像」，此时备份镜像会保存到管理器的内部存储中，这样刷入修补后的镜像后启动，首次打开管理器就会自动将备份移动到 `/data/adb/ksu` 。
:::
...
1. 在 KernelSU 的 [Release 页面](https://github.com/bklynali/BK-KSU/releases) 下载与你手机版本匹配的以 AnyKernel3 开头的 zip 刷机包；如果你不知道下载哪一个，请仔细查阅上述文档中关于 **KMI** 和**安全补丁级别**的描述；下载错误的刷机包很可能导致无法开机，请注意备份。
2. 重启手机进入 TWRP。

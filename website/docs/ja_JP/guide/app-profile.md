...
:::tip ヒント
カスタムの`App Profile`で`NO_NEW_PRIVS`フラグを有効にすることができます。

これにより、プロセスが`su`コマンドを介して権限を昇格させることを防止できます。

ただし、このフラグはKernelSUによるプロセスの権限昇格を**阻止する**だけであり、他のLinuxメカニズムを使用して権限を昇格させることは可能です。

したがって、権限設定には十分注意してください。

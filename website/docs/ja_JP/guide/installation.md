...
1. `LKM`: 端末本来のカーネルを置き換えずに、カーネルに **ローダブルカーネルモジュール** (LKM) を読み込みます。
2. `GKI`: 端末本来のカーネルを KernelSU によって提供される **汎用カーネルイメージ** (GKI) で置き換えます。
...
公式ファームウェアを入手する方法はたくさんあります。端末が `fastboot boot` に対応している場合、最も推奨される最も簡単な方法は、`fastboot boot` を使って KernelSU が提供する GKI カーネルを一時的に起動し、その後マネージャーをインストールして、最後にマネージャー内から直接インストールすることです。この方法では、公式ファームウェアを手動でダウンロードしたり、boot を手動で展開したりする必要がありません。
...
:::tip 純正 boot イメージをバックアップ
マネージャーの「直接インストール」を使うと、増分 OTA の際に一時的に復元できるよう、純正の boot（または init_boot）イメージを自動的にバックアップできます。このバックアップが作成されるのは、現在のスロットがまだ KernelSU によってパッチされていない場合のみです。
バックアップイメージの SHA1 はパッチ済みの boot イメージに保存され、バックアップファイルは `/data/adb/ksu/ksu_backup_$SHA1` に保存されます。
マネージャーの「アンインストール → ストックイメージを復元」を使うと、現在パッチされているイメージに記録された SHA1 に対応するバックアップファイルが存在する場合、そのバックアップが直接復元されます。
3.2.6 以降、root なしで初めて KernelSU をインストールする場合は、「ファイルを選択しパッチを当てる」で boot イメージをパッチしたあとに「純正イメージとしてバックアップ」を選択できます。この場合、バックアップイメージはマネージャーの内部ストレージに保存され、パッチ済みイメージを書き込んで起動したあと、初回起動時にそのバックアップが自動的に `/data/adb/ksu` へ移動されます。
:::
...
使用方法: `ksud boot-patch`。各オプションの詳細はコマンドラインのヘルプで確認できます。
```sh

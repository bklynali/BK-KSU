...
:::tip совет
Вы можете включить флаг `NO_NEW_PRIVS` в своем пользовательском `App Profile`.

Это предотвратит выход процесса за пределы допустимого диапазона и повторное повышение привилегий с помощью команды `su`.

Однако этот флаг **только** предотвращает повышение привилегий процессом с помощью KernelSU; он все еще может выйти за пределы допустимого диапазона, используя другие механизмы Linux.

Поэтому будьте очень осторожны с настройками разрешений.
:::
